...
let UIController = (function () {
  ...
  let formatNumber = function (num, type) {
    ...
    num = Math.abs(num); //Returns absolute val of number
    num = num.toFixed(2); //Method of the number proto..puts 2 decimals on the number we're calling the method on

    numSplit = num.split('.'); // Divdes our num into 2 parts -integer and decimal

    int = numSplit[0];
    if (int.length > 3) {
      int = int.substr(0, int.length - 3) + ',' + int.substr(int.length - 3, 3); //input is 23510, then output is 23,510
    } //More than 1,000
    dec = numSplit[1];
  ...
  return {
    ...
    displayBudget: function (obj) {
      ...
      //obj.budget = obj.budget > 0 ? type = 'inc' : type = 'exp';
